local st, lp, ey, ler, bts, near, col, cw, cd, rat, fr, lst = SysTime, LocalPlayer, EyeAngles, Lerp, {}, { pos = Vector() }, { r = 255, g = 0, b = 0, a = 80 }, Color( 255, 255, 255 ), Color( 40, 40, 40 ), 100, 150
...
hook.Add( "PostDrawTranslucentRenderables", "PreviewBotRoute", function()
...
hook.Add( "HUDPaint", "PaintBotRoute", function()
	if IsValid( near.ent ) and near.ent.vis and not near.ent.nohud then
		...
		local d2, d3 = "Bot Time: " .. bts.con( e.time ) .. bts.gt( e.time ), "Your times:"
...
function ENT:Initialize()
	...
	self.mat = Material( Core.Config.MaterialID .. "/timer.png" )
	self.lastc = 10e10
	self.prev = 0
	self.timed = 0
	...
	if not bts.con then bts.con = Core.GetTimeConvert() end
	if not bts.gt then bts.gt = Core.GetTimeDifference end
...
function ENT:Think()
	if st() - self.timed > 0.1 then
		...
		local dist = (self:GetPos() - lp():GetPos()):Length()
		...
		if dist < (near.pos - lp():GetPos()):Length() then
			local e = near.ent
			if IsValid( e ) and e != self and e.rect then
				local dat = {
				...
				if add and dat.Vel > 80 then
					...
					dat.Text = "- " .. bts.con( dat.Time ) .. " [" .. (dat.DiffTime > 0 and "+" or "-") .. bts.con( math.abs( dat.DiffTime ) ) .. "] (Bot Velocity " .. (dat.DiffVel > 0 and "+" or "") .. math.Round( dat.DiffVel ) .. " u/s)"
...
function ENT:Draw( _, c )
	if not c or not self.draw then return end
	...
	if self.ground then
		...
		if IsValid( self.nextpoint ) then
