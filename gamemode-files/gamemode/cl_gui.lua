...
local tal, tar, tac, tat = TEXT_ALIGN_LEFT, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP
...
function Core.CreateFonts()
	...
	Window.C.GText = Color( 230, 230, 230 )
	Window.C.Text = Color( 185, 185, 185 )
	Window.C.LText = Color( 200, 200, 200 )
	Window.C.DText = Color( 220, 220, 220 )
	Window.C.Opaque = Color( 0, 0, 0, 150 )
	Window.C.BLight = Color( 35, 35, 35 )
	Window.C.BDark = Color( 48, 48, 48 )
end
...
function Window.IsActive( szIdentifier )
	if not Iv( ActiveWindow ) then return false end
	if szIdentifier then return ActiveWindow.Identifier == szIdentifier else return true end
end
...
function Window.Draw( s, w, h )
	...
	DrawBox( 8, 8, 44, w - 16, h - 52, Color( 60, 60, 60 ) )
	DrawBox( 8, 8, 8, w - 16, 44, Color( 80, 80, 80 ) )
	
	DrawColor( Color( 80, 80, 80 ) )
	...
	DrawColor( Color( 66, 66, 66 ) )
	DrawRect( 8, 52, w - 16, s.Separator or 1 )
	
	DrawText( s.Title, "GUIWindowTitle", w / 2, 8 + 44 / 2 + 1, Color( 155, 155, 155, 204 ), tac, tac )
...
local function DrawGrayButton( s, w, h )
	local color, y = Color( 66, 66, 66 ), 0
	if s.IsActive then
		color, y = Color( 46, 46, 46 ), 1
	...
	DrawText( s.Title, s.Font, w / 2, h / 2 - 1 + y, Color( 51, 51, 51, 204 ), tac, tac )
	DrawText( s.Title, s.Font, w / 2, h / 2 - 2 + y, Color( 255, 255, 255 ), tac, tac )
end
...
local function DrawPlainButton( s, w, h )
	...
	if hover or s.Data.Active then
		col = Color( col.r, col.g, col.b, s.Data.over or s.Data.a or 80 )
		if s.Data.a then
			s.Data[ 2 ] = s.Data.b
		end
	...
	if s.Data.Fixed then
		if not s.DrawAt then s.DrawAt = h / 2 end
		DrawText( s.Title, "GUIButtonFont", 10, s.DrawAt, s.Data[ 2 ], tal, tac )
	else
		DrawText( s.Title, "GUIButtonFont", 10, h / 2, s.Data[ 2 ], tal, tac )
	end
	
	if s.Extra then
		DrawText( s.Extra, "GUIButtonFont", w - 24, s.DrawAt, s.Data[ 2 ], tar, tac )
	end
	
	if s.Subtitle1 then
		DrawText( s.Subtitle1, "GUIButtonFont", 10, s.DrawAt + 16, s.Data[ 2 ], tal, tac )
		if s.Subtitle2 then DrawText( s.Subtitle2, "GUIButtonFont", 10, s.DrawAt + 32, s.Data[ 2 ], tal, tac ) end
		if s.Subtitle3 then DrawText( s.Subtitle3, "GUIButtonFont", 10, s.DrawAt + 48, s.Data[ 2 ], tal, tac ) end
	end
	
	if hover and s.Data.List then
		DrawBox( 0, 0, 0, 4, h, Window.C.Opaque )
	end
end

local function DrawCheckbox( s, w, h )
	DrawBox( 0, 0, h / 2 - 10, 20, 20, Color( 70, 70, 70 ) )
	DrawBox( 0, 2, h / 2 - 8, 16, 16, Color( 120, 120, 120 ) )
	DrawBox( 0, 3, h / 2 - 7, 14, 14, s.Checked and Color( 0, 94, 0 ) or Color( 94, 0, 0 ) )
...
function Window.GenerateContent( s, szIdentifier, args )
	...
	elseif szIdentifier == "Nominate" then
		...
		if nLocal != nVersion then
			...
			LoopThrough( maps, function( k, v )
				...
				local name = k .. (k == thismap and " (Current map)" or (isself and " (Previous selection)" or ""))
	...
	elseif szIdentifier == "Vote" then
		...
		for i = 1, 5 do
			...
			local wt = surface.GetTextSize( text[ i ] )
			if wt + s.WidthAdd > s.TopWidth then
				s.TopWidth = wt + s.WidthAdd
			end
		end
		...
		local wt = surface.GetTextSize( text[ #text ] )
		if wt + s.WidthAdd > s.TopWidth then
			s.TopWidth = wt + s.WidthAdd
		end
		
		if s.TopWidth != w then
			w = s.TopWidth
			s:SetSize( w, h )
		end
		
		local txt = "Press and hold " .. Core.GetSettings():ToggleValue( "HUD_CONTEXT" ) .. " to make a selection"
		wt = surface.GetTextSize( txt )
		s.LabelWidth = wt
		
		local lab = Window.Label( s, w / 2 - wt / 2, h - 30, font, txt, Window.C.Text )
		...
		function s:Update( ar )
			...
			for i = 1, 7 do
				...
				wtx = surface.GetTextSize( items[ i ].Title )
				if wtx + self.WidthAdd > self.TopWidth then
					self:ScaleWideTo( wtx + self.WidthAdd )
				end
			end
		...
		timer.Create( "TitleRefresh", 0.1, 0, function()
			...
			local t = math.ceil( math.Clamp( Window.Cache.EndTime - st(), 0, 30 ) )
	...
	elseif szIdentifier == "Records" then
		...
		s.Count = nCount
		s.Title = Core.StyleName( nStyle ) .. " Records (#" .. s.Count .. ")"
		s.Style = nStyle
		...
		local function OnClick( se )
			...
			if se.Expanded then
				...
				if #split > 1 then
					local hold = {}
					table.insert( hold, split[ 2 ] and "Average speed: " .. split[ 2 ] )
					table.insert( hold, split[ 3 ] and "Jumps: " .. split[ 3 ] )
					table.insert( hold, split[ 4 ] and "Strafes: " .. split[ 4 ] )
					table.insert( hold, split[ 5 ] and "Sync: " .. split[ 5 ] .. "%" )
					table.insert( hold, split[ 6 ] and "Perfect: " .. split[ 6 ] .. "%" )
		...
		function s.BuildItems()
			for i = s.Offset, s.Count do
				...
				if s.ViewType == 0 then
					b.Extra = math.Round( tab.nSum, 2 ) .. " Points"
				...
				elseif s.ViewType == 8 then
					...
					if split[ 1 ] then table.insert( b.Subtitles, "Pre: " .. split[ 1 ] .. " u/s" ) end
					if split[ 2 ] then table.insert( b.Subtitles, "Sync: " .. split[ 2 ] .. "%" ) end
					if split[ 3 ] then b.Extra = b.Extra .. " (Strafes: " .. split[ 3 ] .. ")" end
					if split[ 4 ] and tonumber( split[ 4 ] ) > 1 then table.insert( b.Subtitles, "Style: " .. Core.StyleName( tonumber( split[ 4 ] ) ) ) end
					if split[ 5 ] then table.insert( b.Subtitles, "Edge: " .. split[ 5 ] .. " u" ) end
					
					table.insert( b.Subtitles, "Date: " .. Core.ToDate( tab.nDate, true ) )
	...
	elseif szIdentifier == "Ranks" then
		s.Title = s.Title .. " (" .. math.floor( args.Custom[ 2 ] ) .. " pts)"
		...
		for i = 1, #Core.Config.Ranks do
			local col, isc = c % 2 == 0 and Window.C.BLight or Window.C.BDark, args.Custom[ 1 ] == i
			local b = Window.PlainButton( list, (isc and "> " or "") .. Core.Config.Ranks[ i ][ 1 ] .. " (" .. math.floor( Core.Config.Ranks[ i ][ Core.Config.RankColumns[ Core.IsValidBonus( nStyle ) and Core.Config.Style.Bonus or nStyle ] or 3 ] ) .. " pts)", 0, c * 30, w - 10, 30, { col, Core.Config.Ranks[ i ][ 2 ], i, List = true }, OnClick )
			
			b.Think = function( s )
				local mx, my = s:ScreenToLocal( gui.MousePos() )
				if mx >= 0 and my >= 0 and mx <= w - 30 and my <= 30 then
					if not s.HoverTime then
						s.HoverTime = st()
					elseif st() - s.HoverTime > 0.4 then
						if not s.HoverStart then
							s.HoverStart = st()
						end

						local dif = st() - s.HoverStart
						if dif > 6 then
							s.HoverStart = st()
						end
						
						local co = dif > 3 and Lerp( (dif - 3) / 3, 0, 255 ) or Lerp( dif / 3, 255, 0 )
						s.Data[ 1 ] = Color( co, co, co )
						s.Data.over = 255
					end
				else
					s.HoverTime = nil
	...
	elseif szIdentifier == "Bot" then		
		s.DefaultText = args.Custom[ 4 ] or "To start playing a run, left click it. Right click a style for older runs.\n\nRecorded runs:"
		s.HeaderLab = Window.Label( s, 14, 54, "BottomHUDTiny", s.DefaultText, Window.C.Text )
		s.OuterView = false
		
		local styles, details = args.Custom[ 1 ], args.Custom[ 2 ]
		...
		function s.BuildItems()
			...
			for i = 1, #styles do
				...
				local col = c % 2 == 0 and Window.C.BLight or Window.C.BDark
				local highlight = args.Custom[ 3 ] and args.Custom[ 3 ][ 1 ] == details[ i ].Style and args.Custom[ 3 ][ 2 ] == details[ i ].SteamID and args.Custom[ 3 ][ 3 ] == details[ i ].Time
				local b = Window.PlainButton( list, i .. ". " .. styles[ i ] .. " (" .. s.convert( details[ i ].Time or 0 ) .. ")", 0, c * 30, w - 10, 30, { col, highlight and Core.Config.Prefixes.Notification or Window.C.Text, List = true, Fixed = true }, OnClick )
		...
		function s:ThinkHook( bType )
			if bType then return end
			if KeyCheck( KEY_BACKSPACE ) and self.OuterView then
				self:Update( { 2 } )
			end
		end
		...
		function s:Update( varArgs )
			...
			elseif varArgs[ 1 ] == 1 then
				local text = Window.Wrap( "BottomHUDTiny", varArgs[ 2 ], w - 50 )
				self.HeaderLab:SetText( string.Implode( "\n", text ) )
				self.HeaderLab:SetTextColor( Core.Config.Prefixes.Notification )
				
				timer.Remove( "RevertString" )
				timer.Create( "RevertString", 2, 1, function()
					if IsValid( ActiveWindow ) and IsValid( ActiveWindow.HeaderLab ) then
						ActiveWindow.HeaderLab:SetText( ActiveWindow.OuterView and ActiveWindow.HeaderText or ActiveWindow.DefaultText )
						ActiveWindow.HeaderLab:SetTextColor( Window.C.Text )
					end
				end )
			elseif varArgs[ 1 ] == 2 then
				if list.Cache and #list.Cache > 0 then
					...
					for i = 1, #list.Cache do
						...
						local col = c % 2 == 0 and Window.C.BLight or Window.C.BDark
						local b = Window.PlainButton( list, data[ 1 ], 0, c * 30, w - 10, 30, { col, data[ 2 ], List = true, Fixed = true }, self.DOnClick )
						b.Extra = data[ 3 ]
	...
	elseif szIdentifier == "Maps" then
		...
		if s.Type then
			...
			if s.Type == "Left" or s.Type == "NoWR" then
				...
				if nLocal != nVersion then
					Core.Print( "General", "Your map list is outdated. Please open the !nominate menu to update it." )
					return Window.Close()
				else
					...
					for map,data in pairs( maps ) do
						if Core.IsValidBonus( s.Style ) and data.nBonusMultiplier == 0 then continue end
			...
			elseif s.Type == "Beat" then
				...
				for i = 1, s.Count do
					...
					if data then
						local isb = Core.IsValidBonus( s.Style )
	...
	elseif szIdentifier == "Realtime" then
		...
		new.SFunc, new.BA, new.BN, new.TI, new.CT = function( i ) if i > 180 then i = i - 360 elseif i < -180 then i = i + 360 end return i end, bit.band, bit.bnot, table.insert, CurTime
		...
		s.Player.RTSF = function( p, js )
			if js and s.JumpStats then
				...
				print( (js == 0 and "Prestrafe" or "Speed on jump " .. js) .. ": " .. math.Round( vel, 2 ) .. " u/s (+" .. math.Round( s.Stats[ js ][ 1 ] - vel, 2 ) .. ")", s.Convert( stamp ) .. " [+" .. s.Convert( stamp - s.Stats[ js ][ 2 ] ) .. "]", im1 and "New velocity best (-" .. math.Round( im1, 2 ) .. " u/s)" or "", im2 and "New time best (-" .. s.Convert( im2 ) .. ")" or "" )
		...
		function s:ThinkHook()
			...
			self:SetLabel( "Sync", "Sync: " .. string.format( "%.1f", math.Clamp( (data.SA / data.ST) * 100.0 + 12, 0, 100 ) ) .. " / " .. string.format( "%.1f", math.Clamp( (data.SB / data.ST) * 100.0 + 12, 0, 100 ) ) )
			...
			if input.IsKeyDown( KEY_1 ) then
				if not self.LastPress then
					self.MainClose:SetVisible( self.Hidden )
					
					for _,v in pairs( self.Labels ) do
						v:SetVisible( self.Hidden )
					end
		...
		function s:TranslateKeys( num )
			...
			cm.KS = {}
			cm.TI( cm.KS, cm.BA( cm.SBT, IN_FORWARD ) > 0 and "W" or nil )
			cm.TI( cm.KS, cm.BA( cm.SBT, IN_MOVELEFT ) > 0 and "A" or nil )
			cm.TI( cm.KS, cm.BA( cm.SBT, IN_BACK ) > 0 and "S" or nil )
			cm.TI( cm.KS, cm.BA( cm.SBT, IN_MOVERIGHT ) > 0 and "D" or nil )
			cm.TI( cm.KS, cm.BA( cm.SBT, IN_DUCK ) > 0 and "Duck" or nil )
			cm.TI( cm.KS, cm.BA( cm.SBT, IN_ATTACK ) > 0 and "Fire" or nil )
			
			return string.Implode( ", ", cm.KS )
		end
		
		local function OnStatTrack( ply, data )
			if ply != s.Player then return end
		...
		local function OnUpdate()
			...
			ActiveWindow:SetLabel( "ZJumpStats", "Press 1 to hide without closing" )
	...
	elseif szIdentifier == "SSJ" then
		if not Window.Cache.SSJData then
			Window.Cache.SSJData = args.Custom
		end
		
		s.Submit = Window.Cache.SSJData
		s.Content = {
			"1. Toggle [" .. (s.Submit.enabled and "On" or "Off") .. "]",
			"2. Mode [" .. (s.Submit.every == 0 and "Every" or (s.Submit.every == 1 and "6th" or (s.Submit.every == 2 and "Every 6th" or "oops"))) .. "]",
			"3. Show Speed Difference [" .. (s.Submit.speed and "On" or "Off") .. "]",
			"4. Show Height Difference [" .. (s.Submit.height and "On" or "Off") .. "]",
			"5. Show Gain % [" .. (s.Submit.showgains and "On" or "Off") .. "]",
			"0. Close Menu",
		}
		...
		for i = 1, #s.Content do
			s.Commands[ i ] = Window.Label( s, 20, 36 + i * 24, "BottomHUDSemi", s.Content[ i ], Window.C.Text )
		end
		...
		function s:Update( args )
			...
			s.Content = {
				...
				"2. Mode [" .. (data.every == 0 and "Every" or (data.every == 1 and "6th" or (data.every == 2 and "Every 6th" or "oops"))) .. "]",
				"3. Show Speed Difference [" .. (data.speed and "On" or "Off") .. "]",
				"4. Show Height Difference [" .. (data.height and "On" or "Off") .. "]",
				"5. Show Gain % [" .. (data.showgains and "On" or "Off") .. "]",
				"0. Close Menu",
			}

			for i = 1, #s.Content do
				s.Commands[ i ]:SetText( s.Content[ i ] )
			end
		end
	elseif szIdentifier == "TAS" then
		...
		function s:TranslateKeys( num )
			...
			local ba, ti, dt = bit.band, table.insert, {}
			
			ti( dt, ba( num, IN_FORWARD ) > 0 and "W" or nil )
			ti( dt, ba( num, IN_MOVELEFT ) > 0 and "A" or nil )
			ti( dt, ba( num, IN_BACK ) > 0 and "S" or nil )
			ti( dt, ba( num, IN_MOVERIGHT ) > 0 and "D" or nil )
			ti( dt, ba( num, IN_DUCK ) > 0 and "Duck" or nil )
	...
	elseif szIdentifier == "Profile" then
		...
		if tab.Steam then table.insert( s.Content, "Steam: " .. tab.Steam ) end
		if tab.Location then table.insert( s.Content, "Country: " .. tab.Location ) end
		
		if tab.Points and tab.TopPoints and tab.PlayerPos and tab.Players then
			if Core.CountHash( tab.Points ) >= 1 and tab.Points[ 1 ] then
				s.Expands[ 1 ] = {}
			...
			if s.Expands[ 1 ] then
				for i,points in pairs( tab.Points ) do
					...
					if i == 1 then
						table.insert( s.Content, "1. Rank: " .. pos .. " / " .. amount )
					end
		...
		if tab.WRs then
			table.insert( s.Content, "2. Total WRs: " .. tab.WRs )
		...
		if tab.MapsBeat then
			table.insert( s.Content, "3. Maps beat: " .. tab.MapsBeat .. " / " .. tab.MapsTotal )
			s.Expands[ 3 ] = function()
				RunConsoleCommand( "say", "/mapsbeat " .. (ActiveWindow.SavedData.Steam or "") )
			end
		end
		
		if tab.Recent and #tab.Recent > 0 then
			table.insert( s.Content, "4. Most recent record: " .. os.date( "%Y-%m-%d %H:%M:%S", tab.Recent[ 1 ].nDate ) )
			...
			table.insert( s.Content, "4. No maps beaten recently" )
		end
		
		table.insert( s.Content, "" )
		table.insert( s.Content, "0. Close window" )
	...
	elseif szIdentifier == "Settings" then
		...
		local function SpawnItems()
			...
			if not s.Target or s.Target == 1 then
				...
				s.Scroller:AddItem( Window.ImageButton( s, "GUI & HUD options", "icon16/folder_go.png", x, y, nil, 20, function() for i = 1, 3 do s.Menu[ i ].Data.Active = nil end s.Target = 4 s.TTitle = "GUI and HUD options" s.TArray = { "GUI_VISIBILITY", "HUD_DEFTYPE", "HUD_OLDTYPE", "HUD_PLAINTYPE", "HUD_SPECTATOR", "HUD_PERMSYNC", "HUD_VEL3D", "HUD_NOVEL", "HUD_DATETIME", "HUD_NOBLUR" } WindowToggle( { GetParent = function() return s end } ) end ) ) y = y + 30
				s.Scroller:AddItem( Window.ImageButton( s, "Timer options", "icon16/folder_go.png", x, y, nil, 20, function() for i = 1, 3 do s.Menu[ i ].Data.Active = nil end s.Target = 5 WindowToggle( { GetParent = function() return s end } ) end ) ) y = y + 30
				s.Scroller:AddItem( Window.ImageButton( s, "Radio preferences", "icon16/folder_go.png", x, y, nil, 20, function() for i = 1, 3 do s.Menu[ i ].Data.Active = nil end s.Target = 4 s.TTitle = "Radio preferences" s.TArray = { "RADIO_MESSAGES" } WindowToggle( { GetParent = function() return s end } ) end ) ) y = y + 30
				s.Scroller:AddItem( Window.ImageButton( s, "Bot path options", "icon16/folder_go.png", x, y, nil, 20, function() for i = 1, 3 do s.Menu[ i ].Data.Active = nil end s.Target = 4 s.TTitle = "Bot path options" s.TArray = { "TRAIL_BLUE", "TRAIL_RANGE", "TRAIL_GROUND", "TRAIL_VAGUE", "TRAIL_LABEL", "TRAIL_HUD" } WindowToggle( { GetParent = function() return s end } ) end ) ) y = y + 36
				
				s.Scroller:AddItem( Window.Label( s, x, y - 4, "BottomHUDStressL", "Main Toggles", Window.C.Text ) ) y = y + 24
				
				for _,id in pairs( { "CHAT_TIME", "GAME_PLAYERS", "GAME_PLAYER_IDS", "MISC_NOGUNS", "MISC_SAVESTYLE", "MISC_SCORESPECS", "GAME_ZONES", "GAME_NOZONES", "MISC_THIRDPERSON", "MISC_SHOWCONDIFF", "MISC_VIEWINTERP", "MISC_STEADYVIEW" } ) do
				...
				if not table.HasValue( fovs, tostring( GetConVarNumber( "fov_desired" ) ) ) then fovs[ 5 ] = tostring( GetConVarNumber( "fov_desired" ) ) end
				s.Scroller:AddItem( Window.Label( s, bx, by + 152, mainf, "Local FOV:", Window.C.Text ) )
				s.Scroller:AddItem( Window.Combo( s, bx + 88, by + 150, 40, 20, { Default = function() return tostring( GetConVarNumber( "fov_desired" ) ) end, Setter = function( v ) RunConsoleCommand( "fov_desired", tostring( v ) ) Window.Close() end }, fovs, OnCombo ) )
			...
			elseif s.Target == 2 then
				s.Scroller:AddItem( Window.GrayButton( s, "Reset all", w - 118, y, 85, 24, false, function() Window.Close() settings:Misc( "Reset" ) Core.Print( "General", "All statistics and non-essential settings have been reset to default" ) end ) )
				...
				s.Scroller:AddItem( Window.Label( s, x, y, mainf, "Best Long Jump: " .. settings:Get( "MaximumLJ", 0 ) .. " units", Window.C.Text ) ) y = y + 24
				s.Scroller:AddItem( Window.Label( s, x, y, mainf, "Maximum all-time velocity: " .. math.floor( settings:Get( "MaximumVel", 0 ) ) .. " u/s", Window.C.Text ) )
			...
			elseif s.Target == 5 then
				...
				for _,id in pairs( { "NOTIFY_WRSOUND", "NOTIFY_SPECMSG", "NOTIFY_NOTHING", "NOTIFY_LJS", "NOTIFY_TAS", "HUD_STAGE", "NOTIFY_STAGE", "NOTIFY_COLORS", "NOTIFY_LJSTATS", "NOTIFY_STAGETOP", "NOTIFY_STAGESPEC" } ) do
					local tog = settings:GetToggle( id )
					s.Scroller:AddItem( Window.Check( s, tog.Description, x, y, nil, 20, tog, OnCheck ) )
	...
	elseif szIdentifier == "Query" then
		...
		for i = 1, args.Count * 3, 3 do
			...
			if string.sub( tab[ i ], 1, 2 ) == "[[" then
				nd[ #nd ].Name = "ZZZ " .. string.sub( tab[ i ], 3 )
		...
		for i = 1, #nd do
			...
			if nd[ i ].SortAbuse then
				tab[ st + 1 ] = string.sub( tab[ st + 1 ], 5 )
			end
		end
		...
		s:Center()
	elseif szIdentifier == "Radio" or szIdentifier == "Admin" then
		args.Populate( s, args.Custom, Window )
	end
...
local function ReceiveBotUpdate( varArgs )
	if Window.IsActive( "Bot" ) then
		ActiveWindow:Update( varArgs )
	else
		Core.Print( "General", varArgs[ 3 ] )
	end
end
Core.Register( "GUI/UpdateBot", ReceiveBotUpdate )
...
local function ReceiveSSJUpdate( varArgs )
	--PrintTable( varArgs )
	if Window.IsActive( "SSJ" ) then
		--print("active")
		ActiveWindow:Update( varArgs )
	end
end
Core.Register( "GUI/UpdateSSJ", ReceiveSSJUpdate )
