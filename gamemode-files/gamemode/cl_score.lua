...
local admin_specials = { ["76561197991133408"] = "Fun fact: This guy is the owner of Prestige Gaming", ["76561198205782696"] = "Fun fact: This guy is the creator of this gamemode and server" }
local admin_names, medal_names = { "Trusted", "Trusted+", "Junior Admin", "Full Admin", "Super Admin", "Gamemode Manager", "Owner" }, { "Gold Medal", "Silver Medal", "Bronze Medal" }
local icon_muted, icon_setting = Material( "icon32/muted.png" ), Material( "icon16/cog.png" )
...
for i = 1, 13 do icon_rank[ i ] = Material( Core.Config.MaterialID .. "/icon_rank" .. i .. ".png" ) end
for i = 1, 3 do icon_special[ i ] = Material( Core.Config.MaterialID .. "/icon_special" .. i .. ".png" ) end

local function AdminAction( szAction, szSID )
	if not IsValid( lp() ) then return end
	if Core.IsAdminAvailable() or lp():GetObj( "Access", 0 ) > 2 then
...
local function IsGlobalAdmin( ply )
	local access = ply:GetObj( "Access", 0 )
	if access > 2 then
		return access
	end
end
...
local TooltipHelp = {
	...
	["subrank"] = "Shows how far the player has progressed into his rank.\nIt ranges from 0 to 10.\nValue is indicated by amount of diamonds.\nThe golden ring adds another 5 to the value.\nRed Ring: Most WRs on Normal\nGreen Ring: Most WRs obtained on their current style\nLight Blue Ring: If they are top rank and hold the #1 place in !top for their style", ["player"] = "The name of the player", ["timer"] = "This shows the record of the player on the selected style", ["style"] = "This is the style the player is currently playing on\nTo view all styles, type !style",
	["medal"] = "If the record the player has is in the top 3 for that style\na specific medal is awarded", ["position"] = "The position at which the player is placed in the leaderboards", ["ping"] = "The player's latency to the server\n\n0-90: Within America\n91 - 220: Europe\n221 - 999: Australia", ["remotetimer"] = "The time the player has as of opening this expanded window", ["connected"] = "For how long the player has been connected to the server", ["completed"] = "The amount of times the player has finished a course this session",
...
local function DrawPlayerItem( se, w, h )
	...
	elseif valid and not isbot and ply:IsPlayer() then
		...
		if se.SpecialRank > 0 then
			DrawMat( icon_special[ se.SpecialRank ] )
			DrawCol( CWhite )
			DrawTex( o - se.tw - 56, 0, 32, 32 )
...
local function PutPlayerItem( self, list, ply, mw, id, total )
	...
	elseif b then
		btn.RankID = ply:GetObj( "Rank", -1 )
		btn.StyleID = ply:GetObj( "Style", Styles.Normal )
		btn.RankText, btn.RankColor = Core.ObtainRank( btn.RankID, btn.StyleID, true )
		btn.StyleText = StyleName( btn.StyleID )
		btn.Position = ply:GetObj( "Position", 0 )
		btn.Access = ply:GetObj( "Access", 0 )
		btn.SubRank = ply:GetObj( "SubRank", 1 )
		btn.SpecialRank = ply:GetObj( "SpecialRank", 0 )
...
local function ListPlayers( self, list, mw, bots )
	local players = {}
	if bots then
		players = player.GetBots()
		
		for _,p in pairs( players ) do
			p.StyleID = p:GetObj( "Style", 0 )
			...
			elseif p.StyleID > Styles.Normal then
				...
				p.BotType = p:GetObj( "TrueStyle", -1 ) > -1 and 2 or 3
			end
		end
		...
		table.sort( players, function( a, b )
			if a:GetObj( "Style", 1 ) < b:GetObj( "Style", 1 ) then return true end
			if a:GetObj( "Style", 1 ) > b:GetObj( "Style", 1 ) then return false end
			if a:GetObj( "Rank", 1 ) > b:GetObj( "Rank", 1 ) then return true end
			if a:GetObj( "Rank", 1 ) < b:GetObj( "Rank", 1 ) then return false end
			
			local ra, rb = a:GetObj( "Record", 0 ), b:GetObj( "Record", 0 )
	...
	for id,ply in pairs( players ) do
		PutPlayerItem( self, list, ply, mw, id, #players )
	end
...
OpenMenu = function( ply )
	...
	if ply != lp() then	
		if not ply:IsBot() then
			...
			local szURI = ply:GetObj( "ProfileURI", "" )
			if szURI != "" then
