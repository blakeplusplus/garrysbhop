...
-- from czar: this script doesnt really work lol. it never did. but people think it does shit so it's kept
...
local function AcceptInput(self, input, activator, caller, data)
	...
	-- No boosterfix?
	if (not (self.noboosterfix)) then return end
...
local function AddCommand()
	Core.AddCmd({"boosterfix", "fixboosters", "booster"}, function(ply, arguments)
		...
		Core.Print( ply, "Timer", "booster jews have been " .. (ply.noboosterfix and "enabled" or "disabled") .. ".")
	end)
end
