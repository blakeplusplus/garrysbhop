...
--[[
	...
	Used by: The whole spectator module
--]]
...
--[[
	...
	Used by: Command (from scoreboard)
--]]
...
function Spectator.NotifyBot( bot )
	if not Core.BotExists( bot.Style ) then
		Core.NotifyBotRestart( bot.Style )
	end
...
--[[
	...
	Used by: Timer functions (PostTimerCleanup)
--]]
function Spectator.PlayerRestart( ply, nFixed )
	...
	if nFixed then
		...
		if nTimer then
			nTimer = Spectator.GetTime() - nTimer
		end
...
function Spectator.NotifyWatchers( ply, ending )
	...
	local data
	if ply:IsBot() then
		data = Core.GenerateBotNotify( ply, ply.Style, SpectatorList )
...
--[[
	...
	Used by: Commands, timer
	Notes: Now using this method rather than global calls
--]]
