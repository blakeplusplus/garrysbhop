...
local function ReceiveAdmin( varArgs )
	...
	elseif szType == "GUI" then
		...
		local args = varArgs[ 3 ] or Admin.Cached
...
local DrawObj = {}
local Min, Max, lp, st = math.min, math.max, LocalPlayer, SysTime
local DrawMaterial = Material( Core.Config.MaterialID .. "/timer.png" )
local DrawMat, DrawBeam = render.SetMaterial, render.DrawBeam

function DrawCustomZone( obj )
	if not obj.Created or not obj.Color then return end
	
	local W, B, T, C = 3, obj.Bottom, obj.Top, obj.Color
	DrawMat( DrawMaterial )
...
function Admin.DrawAreaEditor()
	if Admin.Editor and Admin.Editor.Active then
		if not DrawObj.Created then
			...
			DrawObj.Color = Color( 50, 0, 255 )
			DrawObj.DrawBox = DrawCustomZone
		end
		
		if DrawObj.DrawBox then
			...
			DrawObj.Bottom = { Vector( m.x, m.y, m.z ), Vector( m.x, x.y, m.z ), Vector( x.x, x.y, m.z ), Vector( x.x, m.y, m.z ) }
			DrawObj.Top = { Vector( m.x, m.y, x.z ), Vector( m.x, x.y, x.z ), Vector( x.x, x.y, x.z ), Vector( x.x, m.y, x.z ) }
			DrawObj:DrawBox()
		end
	else
		DrawObj = {}
