...
local JUMP_POWER, JUMP_DEFAULT = Core.Config.Player.JumpPower, Core.Config.Player.ScrollPower
...
local function OnStaminaMove( ply, mv, cmd )
	...
	if flVelModf < 1 then
		...
		if flVelModf >= 1.0 then
			...
			if spd != 0.0 and spd > (maxspeed * maxspeed) then
				...
				ply.stamspeeds = { mv:GetForwardSpeed(), mv:GetSideSpeed(), mv:GetUpSpeed() }
			end
		end
	...
	if ply:IsOnGround() then
		if cmd:KeyDown( IN_JP ) and (not STAMINA_SET[ ply ] or FN_ST() - STAMINA_SET[ ply ] > 0.1) then
			ply:SetJumpPower( (flStamina == 0 or not STAMINA_STYLES[ ply.Style ]) and JUMP_POWER or JUMP_DEFAULT )
