...
local Style = Core.Config.Style.Normal
local TopVel, TotalVel, CounterVel, MaxVel, LastVel = 0, 0, 0, 0
local TimeBest, TimeTop, TimeBegin, TimeEnd, TimeStage, TimeStageEnd, StageID = 0, {}
local ActiveHUD, ActiveNote, IsSpec, IsSimple, IsDelay, IsCool, Is3DVel, IsShowTime, IsShowStage, IsHideVel, IsDefault, IsSurfline, Markers
local KeyCheck, FColor, StyleName, RankMode, ScrWidth, Iv, DrawText, DrawBox, DrawBoxEx, DrawColor, DrawRect, DrawOut = input.IsKeyDown, Color, Core.StyleName, Core.Config.Modes, ScrW, IsValid, draw.SimpleText, draw.RoundedBox, draw.RoundedBoxEx, surface.SetDrawColor, surface.DrawRect, surface.DrawOutlinedRect
local tal, tar, tac, tat, SimpleFont, SimpleFontS, KeyContext = TEXT_ALIGN_LEFT, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, "FullscreenHeader", 30, KEY_C
...
local Note = { Max = 4, DisplayTime = 8, GetDeath = function( s, l ) local n = SysTime() + (l or s.DisplayTime) if s.LastDeath then if n - s.LastDeath < 2 then n = s.LastDeath + 2 end end s.LastDeath = n return n end, ShouldBeAt = function( s, n, c ) return (c - n) * 40 + 8 end, History = {} }
local Spec = { List = {}, IsRemote = false, Title = "", Mode = 3, Count = 0, Sync = nil, SyncLocal = nil, Window = nil, Background = CBg, Modes = { "First Person", "Chase Cam", "Free Roam" }, Updates = { Resize = 1, Hide = 2 }, Data = { Contains = false, Bot = false, Player = "Unknown", Start = nil, Record = nil } } Spec.DefaultData = table.Copy( Spec.Data )


local function SetStyle( nStyle, bPractice )
	...
	Style = nStyle
	lp().Style = nStyle
	lp().Practice = bPractice
	Spec.Practice = lp().Practice
end

local function GetSpeedData()
	if CounterVel == 0 then return { TopVel, 0 } end
	return { TopVel, TotalVel / CounterVel }
end
...
local function SetFinish( ar )
	...
	local d1, d2, d3 = ar:UInt( 16 ), ar:UInt( 16 ), ar:Double()
	local str = " (" .. d1 .. " jumps, " .. d2 .. " strafes with " .. d3 .. "% sync" .. (ar:Bit() and ", " .. ar:Double() .. " points" or "") .. ")"
...
local function SetRecord( ar )
	...
	if ar:Bit() then SetStyle( ar:UInt( 8 ), ar:Bit() ) end
	if ar:Bit() then Core.Send( "Details", GetSpeedData() ) end
end
Core.Register( "Timer/Record", SetRecord )

local function SetInitial( ar )
	for i = 1, ar:UInt( 8 ) do
		TimeTop[ ar:UInt( 8 ) ] = ar:Double()
	end
end
...
local function SetRankScalar( ar )
	local mp = math.pow
	local e1, e2, e3 = ar:Double(), ar:Double(), ar:Double()
	local Exponential = function( c, n ) return c * mp( n, 2.9 ) end
	
	for i = 1, #Core.Config.Ranks do
		Core.Config.Ranks[ i ][ 3 ] = Exponential( e1, i )
		Core.Config.Ranks[ i ][ 4 ] = Exponential( e2, i )
		Core.Config.Ranks[ i ][ 5 ] = Exponential( e3, i )
	end
end
...
local function SpeedTracker()
	local lpc = lp()
	if not Iv( lpc ) or not TimeBegin then return end
	
	local nSpeed = lpc:GetVelocity():Length2D() or 0
	if nSpeed > TopVel then
		TopVel = nSpeed
		if TopVel > MaxVel then
			MaxVel = TopVel
		end
	end
...
local function BindTracker( ply, bind )
	...
	elseif bind == "+attack" then if IsValid( lp() ) and IsValid( lp():GetActiveWeapon() ) and lp():GetActiveWeapon().OnFireLocal then lp():GetActiveWeapon():OnFireLocal( ct() ) end
...
cvars.AddChangeCallback( "pg_showgui", GUIVisibilityCallback )
...
cvars.AddChangeCallback( "pg_showspec", SpecVisibilityCallback )
...
cvars.AddChangeCallback( "pg_showzones", ZoneVisibilityCallback )
...
function Spec.CanSee()
	return GetConVarNumber( "pg_showothers" )
end

function Spec.SetSee( v, d )
	if d then Spec.DefaultView = v return end
	RunConsoleCommand( "pg_showothers", string.format( "%.0f", v ) )
end
Core.SetSpecVis = Spec.SetSee

function Spec.DoClear( varArgs, bSelf )
	Core.CleanTable( Spec.List )
	Core.CleanTable( Spec.Data )
	...
	if Spec.DefaultView and Spec.CanSee() != Spec.DefaultView and not bSelf then
...
function Spec.Update( s, state, special )
	...
	if x != dx or y != dy then
		if x > 0 and y > 0 then
			s.Window:SetPos( ScrWidth() - x - 20, ScrH() - 60 - y )
		end
...
function Core.SetCheckpointDelay( varArgs )
	if varArgs and varArgs[ 1 ] then
		if type( varArgs[ 1 ] ) == "string" then
			...
			for _,id in pairs( varArgs ) do
				local ply = Entity( id )
				if IsValid( ply ) then
					SetPlayerMarker( ply, true )
					Markers[ ply ] = true
				end
			end
		end
...
function Core.CreateHUD()
	...
	ActiveHUD:SetPos( 0, ScrH() - ActiveHUD:GetTall() )
	...
	ActiveHUD.Think = function( s )
		...
		if lpc:IsTyping() or IsValid( DermaRequest ) then return end
	...
	ActiveHUD.Paint = function( s )
		...
		if IsSpec then
			...
			if Iv( ob ) and ob:IsPlayer() then
				local nStyle, nRank = ob:GetObj( "Style", 1 ), ob:GetObj( "Rank", -1 )
			...
			elseif IsShowStage and StageID then
				local szText = "Stage " .. StageID .. ": " .. ConvertTime( TimeStageEnd and TimeStageEnd or (st() - TimeStage) )
				DrawText( szText, "BottomHUDVelocity", w / 2 + 350, 32, CDark, tac, tac )
				DrawText( szText, "BottomHUDVelocity", w / 2 + 350, 30, CLight, tac, tac )
			end
	...
	ActiveNote:SetSize( ScrWidth() * 0.5, ScrH() * 0.15 )
...
local function BaseHUDDraw()
	...
	elseif IsDelay then
		...
		draw.SimpleText( szText, "ScoreboardMassive", ScrW() / 2, ScrH() / 2 - 150, Color( 0, 120, 255 ), tac, tac )
	end
	
	if not IsSurfline and not IsSimple then return end
	local w, h = ScrWidth(), ScrH()
	...
	if spec then
		...
		if IsSimple then
			...
			if Iv( wep ) and wep.Clip1 then
				local nAmmo = lpc:GetAmmoCount( wep:GetPrimaryAmmoType() )
				if nAmmo > 0 then DrawText( "Ammo: " .. wep:Clip1() .. " / " .. nAmmo, SimpleFont, x, y, CLight, tal, tac ) end
			end
		end
		...
		if IsSurfline then
			...
			if Iv( wep ) and wep.Clip1 then
				...
				local szWeapon = wep:Clip1() .. " / " .. nAmmo
				if nAmmo > 0 then
					DrawText( szWeapon, "BottomHUDVelocity", w - 20, h - 28, CDark, tar, tac )
					DrawText( szWeapon, "BottomHUDVelocity", w - 20, h - 30, CLight, tar, tac )
				end
			end
		end
