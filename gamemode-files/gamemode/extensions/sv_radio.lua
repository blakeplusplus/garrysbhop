...
Radio.Volume = {}
Radio.Tuned = {}
Radio.Limits = { Search = 25, Fetch = 25, Query = 50 }
Radio.IP = ""
Radio.SQL, Radio.Net = SQLPrepare, Core.Prepare

-- All used radio paths and formats
Radio.Path = {
	YouTubePlayer = "http://www.prestige-gaming.org/radio/?id=%s&vol=%d",
...
-- API keys
Radio.Keys = {
	Google = "AIzaSyC1v35-nsPmdGenkZk2JO1lNElKXg7m8wk",
	Soundcloud = "b3f4a5d775edef36dbd94e89710e2cc6"
}
...
function Radio.Init()
	...
	Radio.SQL(
		[[CREATE TABLE IF NOT EXISTS game_radio (
			...
			"nRequests"  INTEGER NOT NULL,
			"nDate"  INTEGER
		);]]
	)
	
	-- Get the main IP
	Radio.IP = GetConVar( "ip" ):GetString() or ""
end
		...
		local data = Radio.GetRecentEntries( function( got, data )
			...
			if got then
				...
				for i = 1, #data do
					...
					ar:UInt( data[ i ].nDuration, 32 )
					ar:UInt( data[ i ].nRequests, 32 )
				end
			end
	...
	http.Fetch( url, function( body )
		...
		if not tab or not tab.pageInfo then
			...
			if tab and tab.error and tab.error.code then
				print( "Radio", "YouTube Search Error occurred", url )
	...
	http.Fetch( url, function( body )
		...
		if not tab or tab.errors then
			...
			if tab and tab.errors and tab.errors[ 1 ] and tab.errors[ 1 ].error_message then
				print( "Radio", "Soundcloud Search Error occurred", url )
...
function Radio.AddFromYouTube( ply, id )
	...
	http.Fetch( url, function( body )
		...
		if not tab or not tab.pageInfo then
			local reason = "."
			if tab and tab.error and tab.error.code then
				print( "Radio", "YouTube Add Error occurred", url )
				PrintTable( tab.error )
...
function Radio.AddFromSoundcloud( ply, query, num )
	...
	http.Fetch( url, function( body )
		...
		if not tab or tab.errors then
			local reason = "."
			if tab and tab.errors and tab.errors[ 1 ] and tab.errors[ 1 ].error_message then
				print( "Radio", "Soundcloud Add Error occurred", url )
				PrintTable( tab.errors )
...
function Radio.GetRecentEntries( callback )
	Radio.SQL(
		"SELECT * FROM game_radio ORDER BY nDate DESC LIMIT " .. Radio.Limits.Fetch
	)( function( data, varArg, szError )
...
Core.Tests = Radio.YoutubeTimeToSeconds
