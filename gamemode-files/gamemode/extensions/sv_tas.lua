...
-- Access functions
local ST, FT, TC = SysTime, FrameTime, Core.CleanTable
...
function TAS.Init()
	-- Create time table
	Prepare(
		[[CREATE TABLE IF NOT EXISTS game_tas (
			"szUID"  TEXT NOT NULL,
			"szMap"  TEXT NOT NULL,
			"nStyle"  INTEGER NOT NULL,
			"nTime"  INTEGER NOT NULL,
			"nReal"  INTEGER NOT NULL,
			"nDate"  INTEGER
		);]]
	)
...
--]]
function TAS.GetPlayerTime( szUID, nStyle )
	local output = 0
...
--]]
function TAS.GetTopTime( nStyle )
	local output, steam
	
	Prepare(
		"SELECT nTime, szUID FROM game_tas WHERE szMap = {0} AND nStyle = {1} ORDER BY nTime ASC LIMIT 1",
		{ game.GetMap(), nStyle }
	)( function( data, varArg, szError )
		if Core.Assert( data, "nTime" ) then
			output = data[ 1 ]["nTime"]
...
--]]
function TAS.RemoveTASTimes( ply, nStyle, tab )
...
Core.RemoveTASTimes = TAS.RemoveTASTimes


--[[
	Description: Loads their previous TAS record
--]]
function TAS.LoadRecord( ply, nStyle, nRecord )
	-- Get the player record
	nRecord = nRecord or TAS.GetPlayerTime( ply.UID, nStyle )
	
	-- For public viewing
	ply.Record = nRecord
	ply.Leaderboard = 0
	ply:SetObj( "Record", ply.Record, true )
	ply:SetObj( "Position", ply.Leaderboard, true )
	...
	-- Remove any medals
	if ply.SpecialRank then
		ply.SpecialRank = nil
		ply:SetObj( "SpecialRank", ply.SpecialRank, true )
	end
end
...
--]]
function TAS.LoadRank( ply )
	-- Set the rank to TAS
	ply.Rank = -20
	ply:SetObj( "Rank", ply.Rank, true )
	
	-- Remove any sub-rank
	ply.SubRank = 0
	ply:SetObj( "SubRank", ply.SubRank, true )
...
function TAS.ValidTimer( ply, ent, bonus )
	...
	local isb = Core.IsValidBonus( ply.Style )
...
function TAS.StartTimer( ply, ent, bonus )
	...
	/*elseif Core.Config.IsBhop and math.abs( ply:GetPos().z - ent.min.z ) > 5 and vel2d > 0 then
		ply:ResetSpawnPosition()
...
function TAS.EndTimer( ply, nReal )
	...
	local nPB = TAS.GetPlayerTime( ply.UID, nStyle )
	...
	-- Get the top time
	local nTop, szTop = TAS.GetTopTime( nStyle )
	
	-- Update their time
	TAS.LoadRecord( ply, nStyle, nTime )
	...
	Core.HandleSpecialBot( ply, "TAS", nTime, { ox, oy, oz, ap, ay, bt } )
end
	...
	for ply,bool in pairs( TAS.Players ) do
		...
		if t != 0 and not Paused[ ply ] then
			if TAS.Times[ ply.Style ] and #TAS.Times[ ply.Style ] > 0 then
				if t > (TAS.Times[ ply.Style ][ 1 ].nTime or 10e10) * 1.25 then
					TAS.SetRecording( ply )
					ActiveTime[ ply ] = 0
...
function TAS.RecordsCommand( ply, args )
	...
	if not TAS.Times[ nStyle ] or TAS.TimeInvalidate[ nStyle ] then
		if TAS.TimeInvalidate[ nStyle ] then
			TAS.TimeInvalidate[ nStyle ] = nil
			
			if TAS.Times[ nStyle ] then
				TC( TAS.Times[ nStyle ] )
			end
		end
		...
		Prepare(
			"SELECT * FROM game_tas WHERE szMap = {0} AND nStyle = {1} ORDER BY nTime ASC LIMIT 50",
			{ game.GetMap(), nStyle }
		)( function( data, varArg, szError )
			if Core.Assert( data, "nTime" ) then
				for j = 1, #data do
	...
	if TAS.Times[ nStyle ] and #TAS.Times[ nStyle ] > 0 then
		Core.Prepare( "GUI/Build", {
			ID = "Top",
			Title = "TAS Records (" .. Core.StyleName( nStyle ) .. ")",
			X = 400,
			Y = 370,
			Mouse = true,
			Blur = true,
			Data = { TAS.Times[ nStyle ] or {}, IsEdit = ply.RemovingTimes, Style = nStyle, ViewType = 4 }
		} ):Send( ply )
	else
		Core.Print( ply, "Timer", Core.Text( "TASCommandWRNone", Core.StyleName( nStyle ) ) )
	end
end
...
function TAS.StyleCommand( ply )
	...
	if not ply.TAS then
		if not ply.InSpawn or ply.Tn or ply.TnF then
