local Radio = {}
Radio.Path = Core.Config.MaterialID .. "/radio.txt"
Radio.Channels = {}
Radio.Controls = {}
Radio.Selected = {}
Radio.SearchList = {}
...
function Radio.Callback( id, v, arg )
	...
	elseif id == "Playlist" then
		...
		elseif v == "Add" then
			if Core.CountHash( Radio.Selected ) == 0 or Radio.ActiveView != Radio.MainView then
			...
			for at,_ in pairs( Radio.Selected ) do
				...
				local put = { [5] = os.date( "%Y-%m-%d", os.time() ) }
		...
		elseif v == "Clear" then
			if Radio.ActiveView == Radio.OwnView then
				if not arg then
					...
					local del = Core.CountHash( Radio.Settings.List or {} )
					Radio.Settings.List = {}
					Radio:Put( "List", Radio.Settings.List )
					Radio.ActiveView = nil
					Radio.Callback( "Playlist", "Toggle" )
...
function Radio.Receive( ar )
		...
		if not ar:Bit() then
			...
			if ar:Bit() then
				for i = 1, ar:UInt( 16 ) do
					Radio.Custom[ #Radio.Custom + 1 ] = { ar:String(), ar:String(), ar:String(), ar:String(), Radio.Time( ar:UInt( 32 ) ), ar:UInt( 32 ) }
				end
			end
		end
	...
	elseif id == 3 then
		local yid = ar:String()
		local data = { ar:String(), ar:String(), "", Radio.Time( ar:UInt( 32 ) ), 1 }
	...
	elseif id == 4 then
		...
		for i = 1, ar:UInt( 16 ) do
			...
			Radio.AddList( Radio.MainView, true, { ar:String(), ar:String(), ar:String(), Radio.Time( ar:UInt( 32 ) ), ar:UInt( 32 ) } )
		end
	end
end
