...
function Race.Init( reload )
	if not reload then
		Prepare(
			[[CREATE TABLE IF NOT EXISTS game_racers (
				"szUID"  TEXT NOT NULL,
				"nStyle"  INTEGER NOT NULL,
				"nWins"  INTEGER NOT NULL,
				"nStreak"  INTEGER NOT NULL
			);]]
		)
	end
end
...
--]]
function Race.GetTopList( style )
	if not Race.TopList[ style ] or Race.Invalidate then
		Race.TopList[ style ] = {}
		Race.Invalidate = nil
		
		Prepare(
			"SELECT nStyle, szUID, nWins, nStreak FROM game_racers WHERE nStyle = {0} ORDER BY nWins DESC LIMIT 50",
			{ style }
		)( function( data, varArg, szError )
	...
	return Race.TopList[ style ]
end
Core.GetRaceTop = Race.GetTopList

--[[
	Description: Deletes items from the race top
--]]
function Race.RemoveRaceItems( ply, nStyle, tab )
...
Core.RemoveRaceItems = Race.RemoveRaceItems
...
function Race.Create( style, plys )
	...
	for _,ply in pairs( plys ) do
		if ply.Spectating or ply.Practice or ply.TAS then
...
function Race.Abandon( tab, ply, command )	
	...
	local count = Core.CountHash( tab.Players )
	if count > 2 then				
...
function Race.Process( tab, force )
	-- Count tables
	local finish, players = Core.CountHash( tab.Finished ), Core.CountHash( tab.Players )
	...
	if (not tab.Abandoned or force) and (finish >= players or players == 2) then
		...
		Core.Print( tab.Players, "Timer", Core.Text( "ActivePlayersFinished", finish, players, force and " (" .. Core.CountHash( tab.Abandoned ) .. " abandoned the race)" or "" ) )
	end
end
...
function Race.ChallengeCommand( ply, args )
	if #args == 0 then
		...
		if IsValid( target ) and target != ply then
			...
			if target.Spectating or target.Practice or target.TAS or ply.Spectating or ply.Practice or ply.TAS then
			...
			Core.Prepare( "Notify", { "Timer", Core.Text( "RaceInvitePopup" ), "email_open_image", 4, Core.Text( "RaceChallenged", ply:Name(), Core.StyleName( ply.Style ), id, id ) } ):Send( target )
		else
...
function Race.ChallengeGroupCommand( ply, args )
	if #args == 0 then
		...
		if args[ 1 ] == "start" then
			...
			if Core.CountHash( players ) >= 2 then
				...
				Core.Print( ply, "Timer", Core.Text( "GroupStartShortage", Core.CountHash( Race.GroupInvite[ ply ].Accepted ), Core.CountHash( Race.GroupInvite[ ply ].Accepted ) ) )
			end
		else
			local target
			for _,p in pairs( player.GetHumans() ) do
			...
			if IsValid( target ) and target != ply then
				...
				-- Send the message
				Core.Print( ply, "Timer", Core.Text( "RaceInviteSent", target:Name() ) )
				Core.Prepare( "Notify", { "Timer", Core.Text( "RaceInvitePopup" ), "email_open_image", 4, Core.Text( "GroupChallenged", ply:Name(), Core.StyleName( Race.GroupInvite[ ply ].Style ) ) } ):Send( target )
			else
				return Core.Print( ply, "Timer", Core.Text( "RaceIncorrectTarget", args.Upper[ 1 ] ) )
			end
		end
...
function Race.AcceptGroupCommand( ply, args )
	...
	if #targets > 1 then
		if not args[ 1 ] or not tonumber( args[ 1 ] ) or not targets[ tonumber( args[ 1 ] ) ] then
			...
			for id,v in pairs( targets ) do
				...
				table.insert( data, "[ID " .. id .. "] " .. v:Name() )
			end
	...
	if not IsValid( target ) then
		...
		Core.Print( target, "Timer", Core.Text( "GroupAcceptStart", ply:Name(), Core.CountHash( Race.GroupInvite[ target ].Accepted ), Core.CountHash( Race.GroupInvite[ target ].Players ) ) )
	end
end
...
function Race.TopListCommand( ply, args )
	...
	local data = Race.GetTopList( nStyle )
	if #data == 0 then
